...
def extract():
    ...
    :return:
    """
    connection = sqlite3.connect("db.sqlite") # соединение с базой данных не закрывается, после получения данных.
    ...
    # лучше вместо select *, указать конкретно, какие элементы требуется получить.
...
def transform(__actors, __writers, __raw_data):
    ...
    for movie_info in __raw_data:
        ...
        # если у __actors actor_id тип число, то и у __writers он такой же, значит, если работает и так, то смысла от int(actor_id), потому что он и так поставляется числом.
        writers_list = [(writer_id, __writers.get(writer_id)) for writer_id in new_writers.split(',')] # одноэлементый список без запятой, не будет списком
        actors_list = [(actor_id, __actors.get(int(actor_id))) for actor_id in raw_actors.split(',')] # тоже самое, без запятой.
